...
export const NetworkModal = ({ pageSupportedChains = SUPPORT_ONLY_FON }: { pageSupportedChains?: number[] }) => {
  ...
  const isBNBOnlyPage = useMemo(() => {
    return pageSupportedChains?.length === 1 && pageSupportedChains[0] === ChainId.FON
  }, [pageSupportedChains])
