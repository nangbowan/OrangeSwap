...
const TotalStaked: React.FC<React.PropsWithChildren<MultiplierProps>> = ({ multiplier }) => {
  ...
  const tooltipContent = (
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of ORG rewards each farm receives, as a proportion of the ORG produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 ORG per block, a 40x farm would receive 40 ORG per block.')}
      </Text>
